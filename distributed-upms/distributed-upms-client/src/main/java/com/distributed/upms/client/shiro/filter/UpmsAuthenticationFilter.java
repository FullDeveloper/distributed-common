...
import com.distributed.upms.client.util.RequestParameterUtil;
...
/**
 * @Author: 周润斌
 * @Date: create in 上午 17:16 2018/1/2 0002
...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance("distributed-upms-client").get("distributed.upms.type");
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        ssoServerUrl.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("distributed-upms-client").get("distributed.upms.appID"));
        // 回跳地址
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        // 判断局部会话是否登录
        String cacheClientSession = RedisUtil.get(DISTRIBUTED_UPMS_CLIENT_SESSION_ID + "_" + session.getId());
        if (StringUtils.isNotBlank(cacheClientSession)) {
            // 更新code有效期
            RedisUtil.set(DISTRIBUTED_UPMS_CLIENT_SESSION_ID + "_" + sessionId, cacheClientSession, timeOut);
            Jedis jedis = RedisUtil.getJedis();
            jedis.expire(DISTRIBUTED_UPMS_CLIENT_SESSION_IDS + "_" + cacheClientSession, timeOut);
            jedis.close();
            // 移除url中的code参数
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        // code校验正确，创建局部会话
                        RedisUtil.set(DISTRIBUTED_UPMS_CLIENT_SESSION_ID + "_" + sessionId, code, timeOut);
                        // 保存code对应的局部会话sessionId，方便退出操作
                        RedisUtil.sadd(DISTRIBUTED_UPMS_CLIENT_SESSION_IDS + "_" + code, sessionId, timeOut);
                        LOGGER.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(DISTRIBUTED_UPMS_CLIENT_SESSION_IDS + "_" + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
                        try {
