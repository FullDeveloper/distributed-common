...
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    public Map getActiveSessions(int offset, int limit) {
        ...
        Jedis jedis = RedisUtil.getJedis();
        // 获取在线会话总数
        long total = jedis.llen(DISTRIBUTED_UPMS_SERVER_SESSION_IDS);
        // 获取当前页会话详情
        List<String> ids = jedis.lrange(DISTRIBUTED_UPMS_SERVER_SESSION_IDS, offset, (offset + limit - 1));
        List<Session> rows = new ArrayList<>();
        for (String id : ids) {
            String session = RedisUtil.get(DISTRIBUTED_UPMS_SHIRO_SESSION_ID + "_" + id);
            // 过滤redis过期session
            if (null == session) {
                RedisUtil.lrem(DISTRIBUTED_UPMS_SERVER_SESSION_IDS, 1, id);
                total = total - 1;
                continue;
            }
    ...
    public int forceout(String ids) {
        ...
        for (String sessionId : sessionIds) {
            ...
            RedisUtil.set(DISTRIBUTED_UPMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), (int) upmsSession.getTimeout() / 1000);
        }
